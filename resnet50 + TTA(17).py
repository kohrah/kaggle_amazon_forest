...
def batch_generator_train(zip_list, img_size, batch_size, is_train=True, shuffle=True):
    ...
    while True:
        ...
        for file, mask in batch_files:
            ...
            image = cv2.imread(file) / 255. #cv2.resize(cv2.imread(file), (img_size,img_size)) / 255.
...
TRAIN_FOLDER = 'C:/g/amazon/train-jpg-res/'
TEST_FOLDER = 'D:/G/Amazon/test-jpg/'
...
x_train, x_holdout, y_train, y_holdout = x_train[:38000], x_train[38000:], y_train[:38000], y_train[38000:]
...
#Model
callbacks = [ModelCheckpoint('amazon_15072017_resnet50_v2.hdf5', monitor='val_loss', save_best_only=True, verbose=2, save_weights_only=False),
             ReduceLROnPlateau(monitor='val_loss', factor=0.1, patience=2, verbose=1, mode='auto', epsilon=0.0001, cooldown=0, min_lr=0.0000001),
...
model = ResNet50(include_top=True, weights='imagenet', input_shape=(IMG_SIZE,IMG_SIZE,3))
optimizer = Adam(0.0000005, decay=0.00000005)
model = make_parallel(model, 2)
model.compile(loss='binary_crossentropy', optimizer=optimizer, metrics=['accuracy', fbeta_score_K])
model.load_weights('amazon_15072017_resnet50_v2.hdf5')
...
x_hld = np.array(x_hld, np.float16) / 255.
...
df_test['tags'] = preds
df_test = df_test[:-57]
df_test.to_csv('submission_CV_0.93.csv', index=False)